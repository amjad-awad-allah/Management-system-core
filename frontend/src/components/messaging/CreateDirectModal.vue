    ...
    <div class="relative z-10 w-full max-w-lg max-h-[85vh] flex flex-col bg-gray-900 rounded-2xl shadow-2xl border border-white/10 overflow-hidden">
      ...
      <div class="px-6 py-4 border-b border-white/10 flex items-center justify-between flex-shrink-0 bg-gray-900/80">
        <div class="flex items-center gap-3">
          <div class="w-10 h-10 rounded-xl bg-blue-600/30 text-blue-400 flex items-center justify-center">
            <svg class="w-6 h-6" fill="none" viewBox="0 0 24 24" stroke="currentColor">
              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 12h.01M12 12h.01M16 12h.01M21 12c0 4.418-4.03 8-9 8a9.863 9.863 0 01-4.255-.949L3 20l1.395-3.72C3.512 15.042 3 13.574 3 12c0-4.418 4.03-8 9-8s9 3.582 9 8z" />
            </svg>
          </div>
          <div>
            <h3 class="text-base font-bold text-white">New Direct Message</h3>
            <p class="text-xs text-gray-400">Select a person to start messaging directly</p>
          </div>
        </div>
      ...
      <div class="p-6 space-y-4 overflow-y-auto flex-1 min-h-0">
        ...
        <div class="relative">
          <input
            ...
            class="w-full rounded-xl bg-gray-800/80 border border-white/10 focus:border-blue-500 focus:outline-none py-2.5 px-4 pl-10 text-sm text-white placeholder-gray-500 transition-colors"
          />
        ...
        <div>
          <div v-if="isLoadingUsers" class="text-center py-8">
            ...
            <p class="text-xs text-gray-400">Loading contacts list...</p>
          </div>
            ...
            <div v-if="filteredStaff.length > 0">
              ...
              <div v-show="isStaffExpanded" class="space-y-1.5">
                <div
                  ...
                  class="flex items-center justify-between px-4 py-2.5 rounded-xl border border-white/5 bg-gray-800/40 hover:bg-blue-900/30 hover:border-blue-500/50 text-gray-300 hover:text-white cursor-pointer transition-all group"
                >
                  ...
                  <span class="text-xs font-medium text-blue-400 opacity-0 group-hover:opacity-100 transition-opacity">Start Chat &rarr;</span>
                </div>
              </div>
            </div>
        ...
        <p v-if="error" class="text-red-400 text-xs bg-red-950/40 border border-red-500/30 p-3 rounded-xl">{{ error }}</p>
      </div>
