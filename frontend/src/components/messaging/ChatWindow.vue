<template>
  <div class="flex flex-col h-full">
    ...
    <!-- ── Channel Header ───────────────────────────────────────── -->
    <div class="flex items-center justify-between px-6 py-4 border-b border-white/10 bg-gray-900/40">
      ...
      <div class="flex items-center gap-2">
        <button
          ...
          class="p-2 rounded-xl text-gray-500 hover:text-red-400 hover:bg-red-900/20 transition-colors"
    ...
    >
      ...
      <template v-else>
        ...
        <div v-if="visibleMessages.length === 0" class="text-center py-12 text-gray-600">
    ...
    <!-- ── Message Input ─────────────────────────────────────────── -->
    <div class="border-t border-white/10 p-4 bg-gray-900/40">

      <!-- File preview -->
      <div v-if="selectedFile" class="mb-3 flex items-center gap-2 px-3 py-2 bg-purple-900/30 rounded-xl border border-purple-500/30">
        ...
        <span class="text-sm text-purple-300 flex-1 truncate">{{ selectedFile.name }}</span>
        <button @click="clearFile" class="text-gray-500 hover:text-white transition-colors">
          <svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
      ...
      <div class="flex items-end gap-3">
        <!-- Attachment button -->
        <label class="flex-shrink-0 p-2.5 rounded-xl text-gray-400 hover:text-purple-400 hover:bg-purple-900/20 transition-colors cursor-pointer">
          <svg class="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15.172 7l-6.586 6.586a2 2 0 102.828 2.828l6.414-6.586a4 4 0 00-5.656-5.656l-6.415 6.585a6 6 0 108.486 8.486L20.5 13" />
          </svg>
          <input
        ...
        <textarea
          ...
          rows="1"
          class="flex-1 resize-none rounded-xl bg-gray-800/60 border border-white/10 focus:border-purple-500 focus:outline-none px-4 py-3 text-sm text-white placeholder-gray-600 transition-colors max-h-32 overflow-y-auto"
        ...
        <button
          ...
          class="flex-shrink-0 p-3 rounded-xl bg-purple-600 hover:bg-purple-500 disabled:opacity-40 disabled:cursor-not-allowed text-white transition-all shadow-lg hover:shadow-purple-500/25"
        >
