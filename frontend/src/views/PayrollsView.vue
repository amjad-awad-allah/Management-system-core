  ...
  <div>
        ...
        <div class="overflow-x-auto">
          <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-800">
            <thead class="bg-gray-50/50 dark:bg-gray-800/50 backdrop-blur-sm">
              <tr>
                <th scope="col" class="py-3.5 pl-4 pr-3 text-left text-sm font-semibold text-gray-900 dark:text-gray-100 sm:pl-6">{{ $t('common.id') }}</th>
                <th scope="col" class="px-3 py-3.5 text-left text-sm font-semibold text-gray-900 dark:text-gray-100">{{ $t('teachers.title') }}</th>
              ...
              <tr v-for="payroll in payrolls" :key="payroll.id" class="hover:bg-gray-50 dark:hover:bg-gray-800/50 transition-colors">
                ...
                <td class="whitespace-nowrap px-3 py-4 text-sm text-gray-900 dark:text-gray-100">
                  ...
                  <div v-if="isEdited(payroll)" class="text-[10px] text-gray-400 dark:text-gray-500 font-normal">
                    {{ $t('common.edit') }}: {{ formatDateTime(payroll.updated_at!) }}
                  </div>
                </td>
                <td class="whitespace-nowrap px-3 py-4 text-sm text-gray-500 dark:text-gray-400">
                  <span :class="[
          ...
          <div class="px-6 py-4 flex items-center justify-between border-t border-gray-200 dark:border-gray-800" v-if="payrolls.length > 0">
             ...
             <div class="space-x-2">
               <button :disabled="currentPage === 1" @click="fetchPayrolls(currentPage - 1)" class="px-3 py-1 text-sm border rounded text-gray-600 disabled:opacity-50 dark:text-gray-300 dark:border-gray-700 cursor-pointer">{{ $t('common.previous') }}</button>
               <button :disabled="currentPage === totalPages" @click="fetchPayrolls(currentPage + 1)" class="px-3 py-1 text-sm border rounded text-gray-600 disabled:opacity-50 dark:text-gray-300 dark:border-gray-700 cursor-pointer">{{ $t('common.next') }}</button>
             </div>
          </div>
        </div>
      </div>
    ...
    <div v-if="showEditModal" class="fixed inset-0 z-50 flex items-center justify-center p-4" aria-labelledby="modal-title" role="dialog" aria-modal="true">
      ...
      <div class="relative z-10 bg-white dark:bg-gray-800 rounded-2xl text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:max-w-lg sm:w-full border border-gray-100 dark:border-gray-700 max-h-[90vh] overflow-y-auto">
          <div class="p-6">
            <h3 class="text-lg font-bold text-gray-900 dark:text-white mb-4">{{ $t('common.edit') }} - {{ editPayrollData.id.substring(0, 8) }}</h3>
              ...
              <div class="space-y-4">
                <div>
                  ...
                  <select v-model="editPayrollData.status" class="mt-1 block w-full rounded-xl border border-gray-300 dark:border-gray-700 bg-white dark:bg-gray-900 text-gray-900 dark:text-white px-3 py-2 sm:text-sm focus:outline-none focus:ring-2 focus:ring-purple-500">
                    ...
                    <option value="Paid">{{ $t('payroll.status.approved') }}</option>
                    <option value="Void">{{ $t('payroll.status.rejected') }}</option>
                  </select>
                </div>
                
                <div>
                  <label class="block text-sm font-medium text-gray-700 dark:text-gray-300">{{ $t('payroll.totalAmount') }} (€)</label>
    ...
    <div v-if="showGenerateModal" class="fixed inset-0 z-50 flex items-center justify-center p-4" aria-labelledby="modal-title" role="dialog" aria-modal="true">
      ...
      <div class="relative z-10 bg-white dark:bg-gray-800 rounded-2xl text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:max-w-lg sm:w-full border border-gray-100 dark:border-gray-700 max-h-[90vh] overflow-y-auto">
        <div class="p-6">
          <h3 class="text-lg font-bold text-gray-900 dark:text-white mb-4">{{ $t('payroll.generateMonthly') }}</h3>
          
          <form @submit.prevent="generatePayroll">
            <div class="space-y-4">
              <div>
                <label class="block text-sm font-medium text-gray-700 dark:text-gray-300">{{ $t('teachers.teacher') }}</label>
                <select v-model="generatePayrollData.teacher_id" required class="mt-1 block w-full rounded-xl border border-gray-300 dark:border-gray-700 bg-white dark:bg-gray-900 text-gray-900 dark:text-white px-3 py-2 sm:text-sm focus:outline-none focus:ring-2 focus:ring-purple-500">
                  <option value="" disabled>-- {{ $t('teachers.teacher') }} --</option>
            ...
            <div class="mt-6 flex justify-end gap-3">
              ...
              <button type="submit" :disabled="isGenerating" class="rounded-xl bg-purple-600 hover:bg-purple-700 disabled:opacity-50 text-white px-4 py-2 text-sm font-semibold shadow-sm transition-colors">
                ...
                <span v-else>{{ $t('payroll.generate') }}</span>
              </button>
            </div>
          </form>
        </div>
    ...
    <div v-if="showPrintModal" class="fixed inset-0 z-50 flex items-center justify-center p-4 print:hidden" aria-labelledby="modal-title" role="dialog" aria-modal="true">
      ...
      <div class="relative z-10 bg-white dark:bg-gray-800 rounded-2xl text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:max-w-4xl sm:w-full border border-gray-100 dark:border-gray-700 max-h-[90vh] flex flex-col">
        ...
        <div class="px-6 py-4 border-b border-gray-100 dark:border-gray-700 flex justify-between items-center bg-gray-50 dark:bg-gray-900/50 shrink-0">
            <h3 class="text-lg font-bold text-gray-900 dark:text-white">{{ $t('payroll.title') }}</h3>
            <div class="flex items-center gap-2">
              <button @click="triggerPrint" class="inline-flex items-center gap-1.5 rounded-xl bg-purple-600 hover:bg-purple-700 text-white px-4 py-2 text-xs font-semibold shadow-sm transition-colors cursor-pointer">
                <svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                  <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17 17h2a2 2 0 002-2v-4a2 2 0 00-2-2H5a2 2 0 00-2 2v4a2 2 0 002 2h2m2 4h10a2 2 0 002-2v-4a2 2 0 00-2-2H9a2 2 0 00-2 2v4h6z" />
                </svg>
                {{ $t('common.print') }}
              </button>
          ...
          <div class="p-8 overflow-y-auto custom-scrollbar bg-white dark:bg-gray-900 flex-1">
            ...
            <div v-else-if="activePayrollDetail" class="space-y-8 text-gray-900 dark:text-gray-100">
              ...
              <div class="flex justify-between items-start border-b border-gray-100 dark:border-gray-800 pb-6">
                <div class="flex items-center gap-4">
                  ...
                  <div>
                    ...
                    <p class="text-xs text-gray-500 dark:text-gray-400 mt-1">Honorar-Abrechnung & Lehrkräfte</p>
                  </div>
                </div>
                <div class="text-right">
                  <h4 class="text-xl font-bold uppercase tracking-wider text-gray-400">{{ $t('payroll.title') }}</h4>
                  <p class="text-md font-mono font-bold mt-1 text-purple-600 dark:text-purple-400">{{ activePayrollDetail.id.substring(0, 8) }}</p>
                </div>
              </div>
              ...
              <div class="grid grid-cols-2 gap-6 text-sm">
                <div>
                  <span class="text-xs text-gray-400 dark:text-gray-500 uppercase tracking-wider block font-medium">{{ $t('teachers.teacher') }}</span>
                  <div class="font-bold text-base mt-1 text-gray-800 dark:text-gray-100">{{ activePayrollDetail.teacher?.name }}</div>
                  <div class="text-xs text-gray-500 dark:text-gray-400 mt-0.5">ID: {{ activePayrollDetail.teacher?.id.substring(0, 8) }}...</div>
                </div>
                <div class="text-right">
                  <div class="space-y-1">
                    <div>
                    ...
                    <div>
                      <span class="text-gray-400">{{ $t('payroll.lessonsCount') }}: </span>
                      <span class="font-bold">{{ activePayrollDetail.total_completed_lessons }}</span>
                    </div>
                    <div>
                      <span class="text-gray-400">{{ $t('payroll.hours') }}: </span>
                      <span class="font-bold font-mono">{{ activePayrollDetail.total_hours }} Std.</span>
                    </div>
                    <div>
                ...
                <table class="w-full text-left border-collapse text-sm">
                  <thead>
                    <tr class="border-b border-gray-200 dark:border-gray-700 text-xs uppercase text-gray-400 font-bold">
                      ...
                      <th class="py-2.5 text-right">{{ $t('payroll.totalAmount') }}</th>
                    </tr>
                  </thead>
                    ...
                    <tr v-for="item in activePayrollDetail.items" :key="item.id">
                      <td class="py-3">
                        <div class="font-semibold text-gray-900 dark:text-white" v-if="item.lesson">
                          {{ new Date(item.lesson.start_time).toLocaleDateString() }}
                        </div>
                        <div v-if="item.lesson" class="text-xs text-gray-500 dark:text-gray-400 mt-0.5">
                          {{ item.lesson.subject?.name }} | {{ item.lesson.room?.name || 'N/A' }}
                        </div>
                      </td>
              ...
              <div class="flex justify-end pt-4 border-t border-gray-100 dark:border-gray-855">
                <div class="w-64 text-right space-y-2">
                  <div class="flex justify-between text-sm">
                    <span class="text-gray-400">{{ $t('payroll.totalAmount') }}:</span>
        ...
        <div class="print-invoice-sheet">
          <div class="header">
            <div>
              ...
              <div class="brand-subtitle">Honorar-Abrechnung & Lehrkräfte</div>
            </div>
            <div class="invoice-title-block">
              <div class="title">ABRECHNUNG</div>
              <div class="number">{{ activePayrollDetail.id.substring(0, 8) }}</div>
            </div>
          </div>
          ...
          <div class="details-grid">
            <div class="bill-to">
              <span class="label">{{ $t('teachers.teacher') }}</span>
              <div class="value name">{{ activePayrollDetail.teacher?.name }}</div>
              <div class="value">ID: {{ activePayrollDetail.teacher?.id.substring(0, 8) }}...</div>
            </div>
            <div class="meta-info">
              <div class="meta-row">
              ...
              <div class="meta-row">
                <span class="label">{{ $t('payroll.lessonsCount') }}:</span>
                <span class="value">{{ activePayrollDetail.total_completed_lessons }}</span>
              </div>
              <div class="meta-row">
                <span class="label">{{ $t('payroll.hours') }}:</span>
                <span class="value font-mono">{{ activePayrollDetail.total_hours }} Std.</span>
              </div>
              <div class="meta-row">
          ...
          <table class="items-table">
            <thead>
              <tr>
                ...
                <th class="right">{{ $t('payroll.totalAmount') }}</th>
              </tr>
            </thead>
            <tbody>
              <tr v-for="item in activePayrollDetail.items" :key="item.id">
                <td>
                  <div class="item-desc" v-if="item.lesson">{{ new Date(item.lesson.start_time).toLocaleDateString() }}</div>
          ...
          <div class="totals-section">
            <div class="total-row">
              <span class="total-label">{{ $t('payroll.totalAmount') }}:</span>
